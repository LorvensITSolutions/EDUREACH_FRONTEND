...
const Email2FAVerify = ({ userId, userEmail, userPassword, onSuccess, onCancel }) => {
  const [code, setCode] = useState(["", "", "", "", "", ""]);
  const [rememberDevice, setRememberDevice] = useState(false);
  const inputRefs = useRef([]);
  const { verifyEmail2FACode, sendEmail2FACode, loading } = useEmail2FAStore();
  ...
  const handleResendCode = async () => {
    ...
    setResending(true);
    try {
      await sendEmail2FACode(userEmail, userPassword);
      // Reset code inputs
      setCode(["", "", "", "", "", ""]);
      if (inputRefs.current[0]) {
        inputRefs.current[0].focus();
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <label htmlFor="rememberDeviceEmail" className="text-sm text-gray-700 cursor-pointer">
      ...
      <div className="mt-4 text-center">
        <button
          ...
          className="text-sm text-blue-600 hover:text-blue-700 hover:underline disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 mx-auto"
        >
          {resending ? (
            <>
