...
const BookActivityChart = () => {
  ...
  useEffect(() => {
    axios.get("/analytics/book-activity-by-month", {})
      .then(res => {
      ...
      .catch(err => {
        ...
        if (err.response?.status !== 401 && err.response?.status !== 403) {
          console.error("Book activity chart error:", err);
        }
    ...
    <div className="w-full max-w-5xl mx-auto bg-white dark:bg-gray-900 shadow-xl rounded-2xl p-6 sm:p-8 animate-fade-in">
      <div className="flex flex-col sm:flex-row items-center justify-between mb-4 gap-3">
        <h2 className="text-2xl font-bold text-primary dark:text-white animate-slide-up">
          📊 Book Activity By Month
        </h2>
        <p className="text-sm font-medium text-gray-700 dark:text-gray-300">
          🎯 Total Activities: <span className="text-accent font-bold">{total}</span>
        </p>
      </div>
...
export default BookActivityChart;
